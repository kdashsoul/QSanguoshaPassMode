      ...
      main_window(main_window),game_started(false)
{
    ...
    connect(ClientInstance, SIGNAL(skills_got(QString)), this, SLOT(chooseSkill(QString)));
...
void RoomScene::chooseSkill(const QString &tab_index)
{
    QDialog *dialog = new QDialog;
        ...
        if(i == 0){
            foreach(const Skill *skill ,Self->getGeneral()->getVisibleSkills()){
                ...
                for(int j = 1 ; j <= skill_attr->values.length() ; j++){
                    ...
                    connect(button, SIGNAL(clicked()), dialog, SLOT(accept()));
                    connect(button, SIGNAL(clicked()), ClientInstance, SLOT(onPlayerMakeSkillChoice()));
    ...
    dialog->setObjectName(".");
    connect(dialog, SIGNAL(rejected()), ClientInstance, SLOT(onPlayerMakeSkillChoice()));
    dialog->setLayout(layout);
...
void RoomScene::onGameOver(){
    ...
    if(! ServerInfo.GameMode.contains("_pass_")){
        ...
        if(victory){
            ...
            result = current < PassMode::maxStage ? "pass" : "win" ;
        }else{
            result = "fail" ;
        }
